...
export const CustomerReviewCandForm = observer(
  ({ onSaved, onCancel }: IProps) => {
    const { candsStore } = useStore();
    const [review, setReview] = useState("");
    const [submitError, setSubmitError] = useState("");

    useEffect(() => {
      setReview(candsStore.candDisplay?.customerReview || "");
    }, []); // eslint-disable-line react-hooks/exhaustive-deps

    const handleSubmit = async () => {
      const res = await candsStore.saveCustomerCandReview(review);
