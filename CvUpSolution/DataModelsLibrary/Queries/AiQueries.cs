...
namespace DataModelsLibrary.Queries
{
    public class AiQueries : IAiQueries
    {
        ...
        public async Task<List<AiCandidateSearchModel>> SearchCvsByPositionFiltered(int positionId, List<int> candidateIds, int limit)
        {
            ...
            // Fetch the stored position embedding as text using ADO.NET
            string? v = null;
            using (var fetchCtx = new cvupdbContext())
            {
                var conn = fetchCtx.Database.GetDbConnection();
                await fetchCtx.Database.OpenConnectionAsync();
                using var cmd = conn.CreateCommand();
                cmd.CommandText = $"SELECT position_embedding::text FROM analyzed_positions WHERE position_id = {positionId} AND position_embedding IS NOT NULL LIMIT 1";
                v = (string?)await cmd.ExecuteScalarAsync();
            }
            ...
            var sql = $@"
                SELECT
                    ...
                    (
                        ...
                        CASE WHEN a.skills_embedding    IS NOT NULL THEN (a.skills_embedding    <=> '{v}') * 0.12 ELSE 0 END +
                        CASE WHEN a.summary_embedding   IS NOT NULL THEN (a.summary_embedding   <=> '{v}') * 0.13 ELSE 0 END +
                        CASE WHEN a.companies_embedding IS NOT NULL THEN (a.companies_embedding <=> '{v}') * 0.05 ELSE 0 END
                    ) / NULLIF(
                        CASE WHEN a.titles_embedding    IS NOT NULL THEN 0.70 ELSE 0 END +
                        CASE WHEN a.skills_embedding    IS NOT NULL THEN 0.12 ELSE 0 END +
                        CASE WHEN a.summary_embedding   IS NOT NULL THEN 0.13 ELSE 0 END +
